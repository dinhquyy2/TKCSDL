...
\begin{longtable}{|p{0.6cm}|p{4cm}|p{2.8cm}|p{6cm}|}
    ...
    \hline
    2 & Chống bán vượt tồn kho & Locking Mechanism & Lock hint (WITH (UPDLOCK, ROWLOCK)) khi Reserve/Confirm/Deduct trên StockBalance \\
    \hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{../docs/ERD/Hinh_ERD_Physical.png}
    \caption{Sơ đồ ERD toàn bộ cơ sở dữ liệu (20 thực thể)}
...
\textbf{Function Details:}
\begin{itemize}
    \item Business Rules (\textbf{quan trọng nhất của toàn hệ thống}): Bảng InventoryTransaction là append-only tuyệt đối — cấm UPDATE/DELETE ở mọi tầng, kể cả truy cập SQL trực tiếp (NFR-SEC-03). Cơ chế thực thi bằng trigger/rule ở tầng SQL Server (chi tiết tại Chương~4, mục~4.2).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\textwidth]{../docs/Sequence/3.5.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\textwidth]{../docs/Sequence/3.6.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\textwidth]{../docs/Sequence/3.7.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\textwidth]{../docs/Sequence/3.8.png}
...
\begin{itemize}
    ...
    \item Data processing: Tính available = on\_hand − reserved theo thời gian thực; khóa dòng StockBalance bằng lock hint (WITH (UPDLOCK, ROWLOCK)) trước khi tăng/giảm reserved hoặc on\_hand.
\end{itemize}

\textbf{Function Details:}
\begin{itemize}
    \item Validation: Nếu available < số lượng đặt mua → từ chối đơn (reject), trả lỗi 409 Conflict.
    \item Business Rules (\textbf{NFR-PERF-02}): Bắt buộc dùng Pessimistic Locking (lock hint \texttt{WITH (UPDLOCK, ROWLOCK)}) hoặc Optimistic Locking (Version Check) ở tầng cơ sở dữ liệu — không được xử lý kiểm tra tồn kho chỉ ở tầng ứng dụng, vì điều này không đủ để chống race-condition khi có nhiều request đồng thời.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\textwidth]{../docs/Sequence/3.9.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\textwidth]{../docs/Sequence/4.0.png}
